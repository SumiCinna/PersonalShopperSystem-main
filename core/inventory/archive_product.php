...
if ($stmt->execute()) {
    log_activity($conn, $_SESSION['user_id'], 'archive', $productId, $product_name, 'status', $old_status, 'archived');
    echo json_encode(['success' => true, 'message' => 'Product archived.']);
} else {
    echo json_encode(['success' => false, 'message' => 'Failed to archive: ' . $stmt->error]);
}
